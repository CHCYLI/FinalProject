...
CREATE TABLE `search` (
  ...
  `one_way` boolean DEFAULT NULL,
  `round_trip` boolean DEFAULT NULL,
  PRIMARY KEY (`customer_id`,`flight_number`),
  CONSTRAINT `search_1` FOREIGN KEY (`customer_id`) REFERENCES `customer` (`customer_id`),
  CONSTRAINT `search_2` FOREIGN KEY (`flight_number`) REFERENCES `flight` (`flight_number`)
